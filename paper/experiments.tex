...
\paragraph{LB submission strategy.}
For the single LB slot available today the conservative play is to
add exp169 to Stream B at low weight rather than replace Tucker.
Concretely we modify the SED-cell of the Mattia fork to compute
$\text{p}_\text{Tucker}$ as the mean over the five Tucker folds,
$\text{p}_\text{exp169}$ as the mean over the five exp169 folds,
and the Stream-B output as
$0.75\,\text{p}_\text{Tucker} + 0.25\,\text{p}_\text{exp169}$.
Under the labeled-SS-train Pearson and the assumption that the
ensembles' test-time quality is comparable, the expected LB delta
is in the range $-0.003$ to $+0.005$, with downside bounded by the
Tucker-only baseline of $0.941$.
