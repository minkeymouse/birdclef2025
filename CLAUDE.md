...
- **`notebooks/birdclef-2026-eos8-*/`** — the Kaggle inference kernels = the actual submission artifacts. Each dir is one kernel: `notebook.ipynb` + `kernel-metadata.json` (declares `dataset_sources` / `model_sources` / `kernel_sources`). Inference runs CPU-only, internet disabled. Two anchor roles: **`eos8-verbatim`** is the clean production reference (LB 0.950); **`eos8-phase4`** is the enriched experiment base (= Karnakbayev Two-Pass SSM — per-class blend + Perch probes + priors; also 0.950) that every lever experiment forks from. The remaining `eos8-<lever>` dirs (`r1`, `hgblend`, `rpair`, `um00`, `lam50`, `ps55/65`, `tax25`, `yz00/10`, …) are single-variable experiments generated by the `*_patch*.py` scripts below — each isolates one knob so its LB delta is attributable. `mattia-fork` is an older 0.949 fork kept for reference.
- **`experiments/sed/`** — Tucker-SED local training **and** autonomous LB-orchestration package. `tucker_spec.py` is the frozen 62-field canonical recipe; `config.py`'s `SedConfig` auto-diffs against it; run `silent_drift_check.py` before any new SED run. `train.py` → `export.py` (ONNX) → `deploy_ensemble.py` (upload + patch kernel). Orchestration scripts (`auto_deploy_ensemble`, `lb_poller`, `lb_processor`) push/poll/submit/record overnight. See `experiments/sed/README.md`.
...
- **`experiments/_data_pipelines/`** — older per-experiment trained outputs (exp169/175/176/ensemble_v3) + Perch embedding caches.
- **`experiments/perch_embed_extract.py`, `lora_dpo_train.py`** — Perch-embedding extraction + LoRA+DPO adapter (the RL-post-training research direction below).
- **`experiments/*_patch*.py` + `poll_*.py` (loose top-level scripts)** — the current iteration loop, lighter-weight than the packaged `sed/` orchestration. `dial_patch.py` / `hg_patch_notebook.py` / `r1_patch_notebook.py` / `rpair_patch.py` programmatically fork `eos8-phase4`'s `notebook.ipynb`, change exactly one constant (or splice in one extra stream), and write a new `eos8-<lever>` kernel dir — so each push is a single-variable LB test. `poll_lb.py` / `poll_score.py` block until the latest Kaggle submission is scored (they strip the CLI `Warning` line). Research/capacity one-offs that use **no** LB slot: `structural_analysis.py` (→ `paper/structural_analysis_2026_06_02.md`), `r1_train_audio_probes.py` (can a probe learn non-Aves from site-invariant train_audio embeddings?), `birdset_finetune.py`, `ns_pseudo_label.py`.
...
transfer under cross-region covariate shift (labeled SS is 68% site-22; test is multi-site), NOT SED quality,
...
## Why the public recipe plateaus at 0.950 (and where the headroom is)
...
But `WEIGHT_CAP = 0.030` limits correction to 3%. For zero-signal species, this is too conservative.
...
| PCEN sidecar gate analysis | 12 Insecta AUC 0.68-1.00, W=0.03 | Weight cap is the limiting factor |
...
| EoS8 + increased PCEN weights | PENDING (submitted) | Unknown |
...
- **Local eval unreliable**: 50/234 species only. Don't gate on local improvement.
- **5 submissions/day**: budget carefully.

## Paper

CLEF 2026 working note. Comp: **2026-06-03**. Paper: **2026-06-17**.
Thesis: covariate shift in cross-region bioacoustic monitoring.
RL post-training analysis could be a novel contribution.
