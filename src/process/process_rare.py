...
`selection.rare_species_threshold`. For each such species it:

1. Gathers all real recordings (down to `audio.min_rating`) not yet in metadata.
2. Supplements with synthetic audio from `paths.synthetic_dir/<species>/*` if available.
...
4. Slides a 10 s window (5 s hop), then saves:
   - `mels/<chunk_id>.npy` – resized mel‑spectrogram (float32)
   - `labels/<chunk_id>.npy` – soft or one‑hot vector
   - metadata row with weight = `labeling.rare_label_weight`

Run after `process_gold.py`; idempotent via shared MD5 hash cache.
"""
...
import utils  # taxonomy, resize_mel, VAD, hash_chunk_id, create_label_vector
...
SYN_DIR = Path(paths_cfg.get("synthetic_dir", "")).expanduser()
...
# Already-processed recordings (skip entirely)
...
def is_silent(x: np.ndarray, db_thresh: float = -50.0) -> bool:
    db = 10 * np.log10(np.maximum(1e-12, np.mean(x ** 2)))
    return db < db_thresh
...
log.info("%d species marked as rare (< %d recordings)", len(rare_species), sel_cfg["rare_species_threshold"])
...
def process_wave(
    ...
    """Slide windows, compute mel, build label vector, save, and record metadata."""
    # trim silence
    ...
    if len(wave) == 0:
        return
    # pad to min duration
    min_samples = int(audio_cfg["min_duration"] * sample_rate)
    if len(wave) < min_samples:
        reps = int(np.ceil(min_samples / len(wave)))
        wave = np.tile(wave, reps)[:min_samples]
    # align to hop
    pad_amt = (hop_samples - len(wave) % hop_samples) % hop_samples
    if pad_amt:
        wave = np.pad(wave, (0, pad_amt))
    total = len(wave)
    ptr = 0
    while ptr + chunk_samples <= total:
        ...
        # build label vector
        if label_cfg.get("use_soft_labels", False):
            lbl = utils.create_label_vector(
                species,
                sec_list or [],
                class_map,
            ...
            lbl[class_map[species]] = 1.0
        np.save(label_path, lbl.astype(np.float32))
        # record metadata
        meta_rows.append(
            {
                "filename": file_id,
                ...
                "weight": float(label_cfg["rare_label_weight"]),
            }
        )
...
for sp in rare_species:
    ...
    for rec in sp_df.itertuples(index=False):
        ...
        if dedup_cfg.get("enabled", False) and h in seen_hashes:
            continue
        seen_hashes.add(h)
        # parse secondary labels for soft vector
        sec_list: List[str] = []
        if label_cfg.get("use_soft_labels", False) and hasattr(rec, "secondary_labels"):
            try:
                sec_list = (
                    ast.literal_eval(rec.secondary_labels)
                    if rec.secondary_labels and rec.secondary_labels != "[]"
                    else []
                )
            except Exception:
                sec_list = []
        process_wave(y, sp, rec_file, sec_list)
    log.info("Processed real recordings for %s", sp)
...
if SYN_DIR.exists():
    for sp in rare_species:
        ...
        if not syn_dir.is_dir():
            continue
        for syn_path in syn_dir.iterdir():
            if not syn_path.is_file():
                continue
            y, _ = librosa.load(syn_path, sr=sample_rate, mono=True)
            h = hashlib.md5(y.tobytes()).hexdigest()
            if dedup_cfg.get("enabled", False) and h in seen_hashes:
                continue
            seen_hashes.add(h)
...
if meta_rows:
    ...
    log.info("No new rare chunks generated.")
