...
def main(cfg_path: str) -> None:
    ...
    arch_cfg = next(
        (a for a in cfg["model"]["architectures"] if a["name"].startswith("regnety")),
        None,
    )
    if arch_cfg is None:
        raise ValueError("RegNetY configuration block missing in YAML file.")
    model_name = arch_cfg["name"]
    num_models = int(arch_cfg.get("num_models", 1))
    pretrained = bool(arch_cfg.get("pretrained", True))

    # Load metadata
    df_meta = pd.read_csv(cfg["dataset"]["train_metadata"])
    if cfg["dataset"].get("include_pseudo", False):
        pseudo = Path(cfg["dataset"]["train_metadata"]).with_name("soundscape_metadata.csv")
        if pseudo.exists():
    ...
    for run in range(1, num_models + 1):
        ...
        if init_ckpt:
            path = f"{init_ckpt}_{run}.pth"
            if Path(path).exists():
                state = torch.load(path, map_location=device)
                model.load_state_dict(state.get("model_state_dict", state), strict=False)
                print(f"Loaded init checkpoint for run {run} from {path}")

        # Configure for trainer
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train RegNetY-800MF ensemble")
    parser.add_argument(
