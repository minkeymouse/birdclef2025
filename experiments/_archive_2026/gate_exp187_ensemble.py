"""Re-gate the exp187 N-fold ENSEMBLE (averages all available fold checkpoints) on the controlled
labeled-SS substrate. Ensemble is stronger than fold-0 alone → may drag less. Run after folds 1,2 (or
more) finish. GATE = Pearson(ensemble,Tucker)<0.93 AND rank-blend(Tucker,ensemble) not-dragging evaluable.
Run: uv run python experiments/gate_exp187_ensemble.py
"""
...
cks = sorted(glob.glob(str(R / "experiments/_data_pipelines/exp187_outputs/**/fold*/best_ckpt.pt"), recursive=True))
print(f"ensemble folds: {len(cks)} -> {[c.split('/')[-2] for c in cks]}")
if not cks: print("no checkpoints"); sys.exit(0)
...
for grp, mask in [("all-eval", np.ones(234, bool)), ("non-Aves", ~aves)]:
    ...
    bl = {w: np.nanmean([A((1 - w) * rp(tuck[:, i]) + w * rp(ens[:, i]), i) for i in ev]) for w in (0.10, 0.15, 0.20)}
    print(f"{grp} ({len(ev)}sp): Tucker={at:.4f} ens={ae:.4f} pearson={pe:.3f} | " +
          " ".join(f"bl{w}={bl[w]:.4f}(Δ{bl[w]-at:+.4f})" for w in (0.10, 0.15, 0.20)))
    if grp == "all-eval":
        print(f"  >>> at w=0.10: {'NON-DRAG' if bl[0.10]>=at-0.0005 else 'DRAGS'} | decorrelated: {pe<0.93}")
print("Decision: submit at the smallest w where Δ≥~0 (non-drag) — the blind-184 effect is only LB-testable (select-best).")
