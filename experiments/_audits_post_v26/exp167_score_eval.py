#!/usr/bin/env python3
"""exp167_score_eval — Score exp167 (multi-region BG SED) ckpt on labeled SS + v33 blend audit (replaces exp50)."""
...
DEVICE = "cuda"
EXP167_CKPT = ROOT / "experiments/_data_pipelines/exp167_outputs/best_ckpt.pt"
SED_N_FFT = 2048; SED_HOP = 512; SED_N_MELS = 256; SED_FMIN = 20; SED_FMAX = 16000  # Tucker config
...
def get_exp167_scores(sc_g, ckpt_path):
    cache = EXP80 / "exp167_scores_labeled.npz"
    if cache.exists():
        return np.load(cache)["scores"]
    print("Running exp167 SED on labeled SS...")
...
def main():
    print("=== exp167b: Score exp167 + v33 blend audit ===\n", flush=True)
    ...
    exp167 = get_exp167_scores(sc_g, EXP167_CKPT)
    print(f"  exp167 scores: {exp167.shape}, range [{exp167.min():.5f}, {exp167.max():.5f}]")

    from scipy.stats import pearsonr
    print(f"\n  Pearson:")
    print(f"    Perch ↔ exp167: {pearsonr(perch_prob.flatten(), exp167.flatten())[0]:.3f}")
    print(f"    exp50 ↔ exp167: {pearsonr(exp50.flatten(), exp167.flatten())[0]:.3f}")
    print(f"    exp121 ↔ exp167: {pearsonr(exp121.flatten(), exp167.flatten())[0]:.3f}")

    ev_mask = sc_g.split.values == "eval"
    rows = [evaluate(v33, v33, ev_mask, Y, sp_taxon, "v33 ref")]

    # Replace exp50 with exp167 in v33 base
    base_new = 0.7 * perch_prob + 0.3 * exp167
    gated_new = apply_v9_gate(base_new, perch_emb, sp_taxon, offset=0.1)
    v33_new = file_max_blend(gated_new, sc_g, alpha=0.10)
    rows.append(evaluate(v33_new, v33, ev_mask, Y, sp_taxon, "v33-style: 0.7P + 0.3*exp167 (full swap)"))

    # Mix exp50 + exp167
    for w_123 in [0.10, 0.15, 0.20]:
        base_mix = 0.7 * perch_prob + (0.3 - w_123) * exp50 + w_123 * exp167
        gated_mix = apply_v9_gate(base_mix, perch_emb, sp_taxon, offset=0.1)
        v33_mix = file_max_blend(gated_mix, sc_g, alpha=0.10)
        rows.append(evaluate(v33_mix, v33, ev_mask, Y, sp_taxon, f"v33-style 0.7P+{0.3-w_123:.2f}exp50+{w_123}exp167"))

    # Additive on top of v33
    for w_123 in [0.05, 0.10, 0.15, 0.20]:
        P = (1 - w_123) * v33 + w_123 * exp167
        rows.append(evaluate(P, v33, ev_mask, Y, sp_taxon, f"v33 + {w_123}*exp167 additive"))
