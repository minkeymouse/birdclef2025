#!/usr/bin/env python3
"""exp167 — Retrain SED with mel-256 / fmin-20 / fmax-16000 (Tucker-style preprocessing).

Motivation: exp50 (mel=128, fmax=14000) misses Insecta 14-16 kHz harmonics.
exp165 ablation: Tucker mel=256 fmax=16000 gives +0.117 macro_d in linear
blend at W=0.30, vs exp50 only +0.008 — 14× gap in same architecture.
This is the data-engineering fix we missed for 4 months. No distillation
(Perch teacher amplifies bird bias, prior failure exp22-28).

Recipe same as exp50 (HGNet-B0 + BCE+BCE + 2025 BG mixing) BUT with
mel-256 / fmin-20 / fmax-16000 (Tucker preprocessing). All other
hyperparameters identical to exp50.

Output: experiments/_data_pipelines/exp167_outputs/
...
OUT = ROOT / "experiments" / "_data_pipelines" / "exp167_outputs"
OUT.mkdir(parents=True, exist_ok=True)

SR = 32000; CLIP_SEC = 20; CLIP_SAMPLES = SR * CLIP_SEC
WINDOW_SEC = 5; WINDOW_SAMPLES = SR * WINDOW_SEC
FILE_SAMPLES = SR * 60
N_FFT = 2048; HOP = 512; N_MELS = 256; FMIN = 20; FMAX = 16000  # Tucker config

BATCH_SIZE = 16; EPOCHS = 20; LR = 1e-3; WD = 1e-2  # batch halved for mel-256
NUM_WORKERS = 4
